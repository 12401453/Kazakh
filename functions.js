...
function loadText() {
  ...
  let text_title_raw = document.getElementById('text_title').value.replaceAll('\u00AD', '').trim();
...
function clearTable() {
 ...
 function texts_link() {
   window.location = "/Kazakh/text_viewer.php";
 }
 function add_text_link() {
  window.location = "/Kazakh/add_texts.php";
}
    ...
    xhttp.onload = () => {
      if (xhttp.readyState == 4) {
         ...
         */
          ...
          //need to set pos_initial again as its fucking up
        }
...
//this only updates tooltips for words sharing the word_engine_id of the word being updated by lemmaRecord()
